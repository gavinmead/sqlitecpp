...
// there were no errors. Most other result codes indicate an error.
constexpr SQLITE_RESULT SQLITE_OK = 0;

// The SQLITE_CANTOPEN result code indicates that SQLite was unable to open a
// file. The file in question might be a primary database file or one of several
...
constexpr SQLITE_PAGE_SIZE SQLITE_PAGE_SIZE_65536 = 1;
...
class SQLiteConnection {
 public:
  explicit SQLiteConnection(
      std::string filename,
      SQLITE_PAGE_SIZE pageSize = SQLITE_PAGE_SIZE_DEFAULT)
 ...
 private:
  ...
  void
  update_last_message(SQLITE_RESULT result, bool isError, std::string message) {
  ...
  std::string db_file;
  bool opened{false};
  std::optional<std::unique_ptr<std::vector<std::byte>>>
      db_file_mmap; // We make optional to delay initialization until open
