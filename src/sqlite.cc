...
SQLITE_RESULT SQLiteConnection::open() {
  ...
  if (!fs::exists(this->db_file)) {
    ...
    int db_file_descriptor =
        ::open(this->db_file.c_str(), O_RDWR | O_CREAT, 0664);
    if (db_file_descriptor == -1) {
      auto result = SQLITE_CANTOPEN;
      this->update_last_message(
          result, is_error(result), "could not open file");
      return result;
    }
    this->opened = true;
    ...
    result = SQLITE_OK;
    this->update_last_message(
        result, is_error(result), "new database created successfully");
    return result;
    ...
    this->update_last_message(
        result, is_error(result), "database opened successfully");
    return result;
  }
}
...
SQLITE_RESULT SQLiteConnection::setup_db_mmap(
    int db_file_descriptor,
    bool is_new_db) {
  // TODO: initialize the DB internals
  ...
  if (is_new_db) {
    if (ftruncate(db_file_descriptor, size) == -1) {
      auto result = SQLITE_CANTOPEN;
      this->update_last_message(
          result, is_error(result), "could not initialize empty db file");
      return result;
    }
  } else {
  ...
  void* mapped = mmap(
      nullptr, size, PROT_READ | PROT_WRITE, MAP_SHARED, db_file_descriptor, 0);
  if (mapped == MAP_FAILED) {
    auto result = SQLITE_CANTOPEN;
    this->update_last_message(
        result,
        is_error(result),
        "could not mmap file: " + std::to_string(errno));
    return result;
  }
